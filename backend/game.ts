...
import { GameTrading, MonopolyMode, MonopolyModes, historyAction, PlayerStats, GameStats } from "./types";
...
type PlayerActionArgs =
    | { action: "buy" }
    | { action: "buy-advance"; newCount: 1 | 2 | 3 | 4 | 5; housesAdded: number; propertyPosition?: number }
    | { action: "sell-advance"; propertyPosition?: number }
    | { action: "skip" };
...
export async function main(playersCount: number, f?: (host: string, Server: Server) => void) {
    ...
    const Spectators = new Map<string, Socket>();
    ...
    const server_histories: Array<historyAction> = [];
    ...
    function initPlayerStats(player: Player) {
    ...
    let chanceGetOutOwner: string | null = null;
    let chestGetOutOwner: string | null = null;
    ...
    function EmitAll(event: string, args: any) {
        ...
        for (const s of Array.from(Spectators.values())) s.emit(event, args);
    }
    function EmitExcepts(id: string, event: string, args: any) {
        ...
        for (const [k, s] of Array.from(Spectators.entries())) if (k !== id) s.emit(event, args);
    }
    ...
    function resolveCard(
        ...
        card: any,
        rolls: number
    ): { requiresPurchaseDecision: boolean; newPosition?: number; extraRoll?: [number, number]; landingNote?: string } {
        switch (card.action) {
            case "addfunds":
        ...
        (socket: Socket, server: Server) => {
            ...
            socket.on("spectator", (name: string) => {
                try {
                    ...
                    if (isSpecReconnecting) {
                        ...
                        if (oldSocket && oldSocket !== socket) oldSocket.disconnect();
                    } else {
                        Spectators.set(socket.id, socket);
                    }
            ...
            socket.on("name", (name: string) => {
                try {
                    ...
                    // ── Select Icon/Color ──
                    socket.on("select_icon", (iconIndex: number) => {
                        try {
                    ...
                    socket.on("roll_dice", () => {
                        try {
                            ...
                            if (player.hasRolled && !player.allowRollAgain) return;
                            ...
                            if (rolledPosition === 30) {
                                ...
                                if (prop && CARD_TILES.has(prop.id ?? "")) {
                                    ...
                                    pendingCard = {
                                        ...
                                        extraRoll: result.extraRoll ?? null,
                                        pendingCard: (result as any).pendingCard ?? null,
                                    };
                                    requiresPurchaseDecision = result.requiresPurchaseDecision;
                    ...
                    socket.on("player_action", (args: PlayerActionArgs) => {
                        try {
                            ...
                            if (args.action === "skip") {
                                if (!selectedMode.allowAuctions) return;
                                ...
                                if (landedProp && landedProp.price !== undefined && landedProp.group !== "Special") {
                                    const isUnowned = Array.from(Clients.values()).every(
                    ...
                    socket.on("auction-bid", (args: { bid: number }) => {
                        try {
                            if (!currentAuction) return;
                            if (player.isBankrupt) return;
                            if (args.bid <= currentAuction.currentBid) return;
                            if (args.bid > player.balance) return;
                    ...
                    function validateAndExecuteTrade(x: GameTrading): boolean {
                        if (!selectedMode.AllowDeals) return false;
                        ...
                        for (const offer of x.turnPlayer.prop) {
                            ...
                            if (!owned) return false; // tp doesn't own this property!
                            if (hasGroupBuildings(tp, offer.group)) return false; // group has buildings!
                        }
                        ...
                        for (const offer of x.againstPlayer.prop) {
                            ...
                            if (!owned) return false; // ap doesn't own this property!
                            if (hasGroupBuildings(ap, offer.group)) return false; // group has buildings!
                        }
                        ...
                        ap.properties = ap.properties.filter((v1: any) =>
                            !x.againstPlayer.prop.some((v2) => v2.posistion === v1.posistion));

                        const aGets = tp.properties.filter((v1: any) =>
                            x.turnPlayer.prop.some((v2) => v2.posistion === v1.posistion));
                        tp.properties = tp.properties.filter((v1: any) =>
                            !x.turnPlayer.prop.some((v2) => v2.posistion === v1.posistion));
