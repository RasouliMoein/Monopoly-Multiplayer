...
async function main(playersCount, f) {
     ...
     */
    function getExpectedRentExposure(player, oldPos) {
        const probs = {
            2: 1 / 36, 12: 1 / 36,
            3: 2 / 36, 11: 2 / 36,
            4: 3 / 36, 10: 3 / 36,
        ...
        for (let roll = 2; roll <= 12; roll++) {
            ...
            if (prop.id === "incometax") {
                ...
                // Find owner
                const clientOwner = Array.from(Clients.values()).find((c) => c.player.properties.some((p) => p.posistion === targetPos) && !c.player.isBankrupt);
                if (clientOwner && clientOwner.player.id !== player.id) {
                    ...
                    if (prp && prp.morgage !== true && prp.morgage !== "true") {
                        if (prop.group === "Utilities") {
                            const ownedCount = owner.properties.filter((op) => propertyByPosition.get(op.posistion)?.group === "Utilities").length;
                            rentOrTax = 7 * (ownedCount === 2 ? 10 : 4);
                        }
                        else if (prop.group === "Railroad") {
                            const ownedCount = owner.properties.filter((op) => propertyByPosition.get(op.posistion)?.group === "Railroad").length;
     ...
     */
    function processLanding(player, position, rolls, multiplier = 1, isCardMove = false) {
        const prop = propertyByPosition.get(position);
        if (!prop)
