...
class GameState {
    ...
    currentId = "";
    gameStarted = false;
    selectedMode = game_1.MonopolyModes[0];
    hostId = "";
    ...
    creditorMap = new Map();
    debugDiceOverrideMap = new Map();
    pendingBankruptMap = new Map(); // creditorId → bankruptId
    pendingTradeMortgages = new Map();
    debtAmountMap = new Map();
    // Housing & hotel pool
    bankHouses = 32;
    bankHotels = 12;
    ...
    calculateNetWorth(player) {
        ...
        for (const prop of player.properties) {
            const propData = Board_1.propertyByPosition.get(prop.posistion);
            if (!propData)
                continue;
    ...
    getExpectedRentExposure(player, oldPos) {
        ...
        for (let roll = 2; roll <= 12; roll++) {
            ...
            const prop = Board_1.propertyByPosition.get(targetPos);
            if (!prop)
                continue;
            let rentOrTax = 0;
            if (prop.id === "incometax") {
                ...
                const clientOwner = Array.from(this.clients.values()).find((c) => c.player.properties.some((p) => p.posistion === targetPos) && !c.player.isBankrupt);
                if (clientOwner && clientOwner.player.id !== player.id) {
                    ...
                    if (prp && prp.morgage !== true && prp.morgage !== "true") {
                        if (prop.group === "Utilities") {
                            const ownedCount = owner.properties.filter((op) => Board_1.propertyByPosition.get(op.posistion)?.group === "Utilities").length;
                            rentOrTax = 7 * (ownedCount === 2 ? 10 : 4);
                        }
                        else if (prop.group === "Railroad") {
                            const ownedCount = owner.properties.filter((op) => Board_1.propertyByPosition.get(op.posistion)?.group === "Railroad").length;
    ...
    processLanding(player, position, rolls, multiplier = 1, isCardMove = false) {
        const prop = Board_1.propertyByPosition.get(position);
        if (!prop)
            return { requiresPurchaseDecision: false, landingNote: "" };
        if (Board_1.INERT_TILES.has(prop.id ?? ""))
            return { requiresPurchaseDecision: false, landingNote: "" };
        if (Board_1.CARD_TILES.has(prop.id ?? ""))
            return { requiresPurchaseDecision: false, landingNote: "" };
        if (prop.id === "gotojail")
            return { requiresPurchaseDecision: false, landingNote: "" };
        const getRentLuckWeight = (pOwner, pos) => {
            ...
            if (prp.count === "h")
                return 3;
            if (typeof prp.count === "number" && prp.count > 0)
                return 2;
            return 1;
        };
        let expectedRent = 0;
    ...
    resolveCard(player, card, rolls) {
        switch (card.action) {
            ...
            case "move": {
                ...
                let passedGo = false;
                if (card.tileid) {
                    targetPos = Board_1.propertyById.get(card.tileid)?.posistion;
                ...
                if (prop && Board_1.CARD_TILES.has(prop.id ?? "")) {
                    ...
                    this.emitServerHistory(`${player.username} landed on ${prop.id === "chance" ? "Chance" : "Community Chest"} space and drew card: "${nextCard.title}"`);
    ...
    startAuction(position) {
        ...
        if (!prop)
            return;
        if (this.auctionIntervalId)
            clearInterval(this.auctionIntervalId);
        this.currentAuction = {
    ...
    endAuction() {
        ...
        const prop = Board_1.propertyByPosition.get(auction.propertyPosition);
        ...
        if (prop.group && prop.group !== "Special") {
            const groupProps = monopoly_json_1.default.properties.filter((p) => p.group === prop.group);
            const ownedGroup = winner.properties.filter((p) => p.group === prop.group);
    ...
    declareBankruptcyForPlayer(targetId) {
        const clientItem = this.clients.get(targetId);
        if (!clientItem)
            return;
        const bpPlayer = clientItem.player;
        this.logFunction(`[BANKRUPTCY] Player ${bpPlayer.username} is declaring bankruptcy. Balance: ${bpPlayer.balance}`);
        ...
        if (creditor !== "bank") {
            const creditorClient = this.clients.get(creditor);
            if (creditorClient) {
                ...
                if (bpPlayer.getoutCards > 0) {
                    ...
                    this.emitServerHistory(`${cp.username} received ${bpPlayer.getoutCards} Get Out of Jail Free card(s) from ${bpPlayer.username}`);
                ...
                for (const prp of bpPlayer.properties) {
                    ...
                    if (prp.count === "h") {
                        ...
                        this.emitServerHistory(`${cp.username} received $${refund} from ${prp.count} house(s) sold on ${propName}`);
                    }
                    prp.count = 0;
                ...
                const mortgagedPending = cp.properties
                    ...
                    .map((prp) => {
                    const propData = Board_1.propertyByPosition.get(prp.posistion);
                ...
                if (mortgagedPending.length > 0) {
                    this.pendingBankruptMap.set(creditor, targetId);
    ...
    validateAndExecuteTrade(x) {
        if (!this.selectedMode.AllowDeals)
            return false;
        ...
        for (const prp of tGets) {
            if (prp.group && prp.group !== "Special") {
                const groupProps = monopoly_json_1.default.properties.filter((p) => p.group === prp.group);
                const ownedGroup = tp.properties.filter((p) => p.group === prp.group);
                if (groupProps.length > 0 && ownedGroup.length === groupProps.length) {
        ...
        for (const prp of aGets) {
            if (prp.group && prp.group !== "Special") {
                const groupProps = monopoly_json_1.default.properties.filter((p) => p.group === prp.group);
                const ownedGroup = ap.properties.filter((p) => p.group === prp.group);
                if (groupProps.length > 0 && ownedGroup.length === groupProps.length) {
