...
const sockets_ts_1 = require("../sockets.ts");
const monopoly_json_1 = __importDefault(require("../monopoly.json"));
async function main(host, initials) {
    const socket = await (0, sockets_ts_1.io)(host);
    const propretyMap = new Map(monopoly_json_1.default.properties.map((obj) => {
        return [obj.posistion ?? 0, obj];
    }));
    // @ts-ignore
    let currentId = "";
    let clients = new Map();
    let selectedMode = types_ts_1.MonopolyModes[0];
    // @ts-ignore
    let trade = undefined;
    //#region engineref
    const engineRef = {
        ...
        setStreet: (args) => {
            ...
            if (x && args.location !== -1 && args.location < 40 && args.location >= 0) {
                function searchForButtons(advanced, location, fartherInfo) {
                    if (advanced) {
                        ...
                        const genereatedActions = {};
                        const count = transformCount(localPlayer.properties[propId].count);
    ...
    const socket_Initials = (args) => {
        ...
        for (const x of args.other_players) {
            clients.set(x.id, new player_1.Player(x.id, x.username).recieveJson(x));
        }
        selectedMode = args.selectedMode;
    };
    const socket_NewPlayer = (args) => {
        clients.set(args.id, new player_1.Player(args.id, args.username).recieveJson(args));
    };
    ...
    function playerMoveGENERATOR(final_position, _xplayer, get200whengo = true, afterFinished, adding = true) {
        ...
        console.log(`${initials.name}`, `${new Date().toTimeString()} generator ${Math.random()} target ${final_position} time ${time} current ${_xplayer.position}`);
                        ...
                        const actionList = engineRef.setStreet({
                            ...
                            onResponse: (b, info) => {
                                ...
                                if (b === "buy") {
                                    ...
                                    socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} bought ${proprety.name}`));
                                }
                                else if (b === "advance-buy") {
                                    const propId = Array.from(new Map(localPlayer.properties.map((v, i) => [i, v])).entries()).filter((v) => v[1].posistion === location)[0][0];
                                    const _info = info;
                                    ...
                                    socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} advanced ${proprety.name}`));
                                }
                                else if (b === "someones") {
                                    const players = Array.from(clients.values());
                                    for (const p of players) {
                                        for (const prp of p.properties) {
                                            if (prp.posistion === location) {
                                                ...
                                                socket.emit("history", (0, types_ts_1.history)(`
                                    ...
                                    if (proprety?.id === "incometax") {
                                        ...
                                        socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} payed income taxes`));
                                    }
                                    if (proprety?.id === "luxerytax") {
                                        localPlayer.balance -= 100;
                                        socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} payed luxery taxes`));
                                    }
                                }
                                    ...
                                    socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} bought ${prp?.name ?? "unkown place"} with rent of ${calculateRent}`));
                                }
                                setTimeout(() => {
                                    clients.set(socket.id, localPlayer);
                                    const json = clients.get(socket.id).toJson();
    ...
    const socket_ChorchResult = (args) => {
        ...
        setTimeout(() => {
            ...
            if (xplayer === undefined)
                return;
            function addBalanceToOthers(amnout) {
                if (xplayer === undefined)
                    return 0;
                ...
                if (xplayer.id === socket.id) {
                    if (amnout > 0) {
                        // give money
                        socket.emit("history", (0, types_ts_1.history)(`${xplayer.username ?? "unknown user"} gave ${amnout} money to [${other_players.map((v) => v.username).join(", ")}]`));
                    }
                    else {
                        // get money!
                        socket.emit("history", (0, types_ts_1.history)(`${xplayer.username ?? "unknown user"} recieve ${-amnout} money from [${other_players
                            .map((v) => v.username)
                            .join(", ")}]`));
                    }
                }
            ...
            switch (c.action) {
                case "move":
                    if (c.tileid) {
                        const p = new Map(monopoly_json_1.default.properties.map((obj) => {
                            return [obj.id, obj];
                        }));
                ...
                case "movenearest":
                    ...
                    const arr = monopoly_json_1.default.properties.filter((v) => v.group === p).map((v) => v.posistion);
                                ...
                                engineRef.setStreet({
                                    ...
                                    onResponse: (b, info) => {
                                        if (b === "buy") {
                                            ...
                                            const json = clients.get(socket.id).toJson();
                                            socket.emit("finish-turn", json);
                                            socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} bought ${prp?.name ?? "unkown place"}`));
                                        }
                                        else if (b === "special_action") {
                                                ...
                                                for (const prp of p.properties) {
                                                    if (prp.posistion === location) {
                                                        ...
                                                        if (proprety.group === "Utilities" && prp.rent) {
                                                            ...
                                                            socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} rolled [${l[0]}, ${l[1]}]`));
                                                            engineRef.diceResults({
                                                                l: [l[0], l[1]],
                                                                time: 2000,
                                                                onDone: () => {
                                                                    ...
                                                                    socket.emit("history", (0, types_ts_1.history)(`${clients.get(socket.id)?.username ?? "unknown player"} pay ${payment_ammount} to ${clients.get(p.id)?.username ?? "unknown player"}`));
                                                                    clients.set(socket.id, xplayer);
                                                                    // engineRef.freeDice();
                                                                    const json = clients.get(socket.id).toJson();
                                                                    socket.emit("finish-turn", json);
                ...
                case "propertycharges":
                    ...
                    var payment_ammount = (c.buildings ?? 1) * sum(xplayer.properties.filter((v) => typeof v.count === "number").map((v) => v.count)) +
                        (c.hotels ?? 1) * xplayer.properties.filter((v) => v.count === "h").length;
                    console.log(`${initials.name}`, `
${(c.buildings ?? 1) * sum(xplayer.properties.filter((v) => typeof v.count === "number").map((v) => v.count))} + 
${(c.hotels ?? 1) * xplayer.properties.filter((v) => v.count === "h").length} 
which is ${payment_ammount}
            ...
            if (time_till_finish >= 0) {
                setTimeout(() => {
                    ...
                    if (xplayer.id === socket.id) {
                        // engineRef.freeDice();
                        socket.emit("finish-turn", clients.get(socket.id).toJson());
                    }
                }, time_till_finish);
            }
        }, numOfTime);
    };
    function socket_Mouse(args) {
    ...
    function socket_playerUpdate(args) {
