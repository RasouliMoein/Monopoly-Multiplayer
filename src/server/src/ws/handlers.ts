...
export function registerSocketHandlers(socket: Socket, server: Server, state: GameState, maxPlayers: number) {
    ...
    state.emitAll = (event: string, args: any) => {
        for (const x of Array.from(state.clients.values())) x.socket.emit(event, args);
        for (const s of Array.from(state.spectators.values())) s.emit(event, args);
    };

    state.emitExcepts = (id: string, event: string, args: any) => {
        for (const [k, x] of Array.from(state.clients.entries())) if (k !== id) x.socket.emit(event, args);
        for (const [k, s] of Array.from(state.spectators.entries())) if (k !== id) s.emit(event, args);
    };
    ...
    socket.on("name", (name: string) => {
        try {
            ...
            else state.emitExcepts(socket.id, "player_update", { playerId: player.id, pJson: player.to_json() });
            ...
            socket.on("player_action", (args: any) => {
                try {
                    ...
                    if (args.action === "buy") {
                        ...
                        if (toNum2(player.properties[idx].count) < maxCount) return;
                        ...
                        if (currentCount === "h") {
                            ...
                            player.properties[idx].count = (currentCount - 1) as 0 | 1 | 2 | 3 | 4;
                            state.emitServerHistory(`${player.username} demoted ${targetProp.name}`);
                        }
                        player.balance += refund;
                    }
                    if (args.action === "skip") {
                        if (!state.selectedMode.allowAuctions) return;
            ...
            socket.on("mortgage_action", (args: any) => {
                try {
                    ...
                    if (args.action === "mortgage") {
                        if (ownedProp.morgage === true) return;
            ...
            socket.on("auction-bid", (args: { bid: number }) => {
                try {
                    ...
                    if (player.isBankrupt) return;
                    if (args.bid <= state.currentAuction.currentBid) return;
