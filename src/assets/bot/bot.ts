...
export async function main(host: string, initials: botInitial, externalToken?: string) {
    const socket = await io(host, externalToken);
                ...
                function searchForButtons(
                    ...
                    if (advanced) {
                        ...
                        for (let index = count + 1; index < 6; index++) {
                            ...
                            if (ownsFullSet && !((index - count) * (_property.housecost ?? 0) > (localPlayer.balance ?? 0))) {
