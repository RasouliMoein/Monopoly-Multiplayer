    ...
    (prop, ref) => {
        useImperativeHandle(ref, () => ({
            message(message, type, time, after, sfx) {
                ...
                const animation_text = "popoff .7s cubic-bezier(.62,.25,1,-0.73)";
            ...
            dialog(build_dialog_function, soundtrack) {
                ...
                switch (soundtrack) {
                    case "loosing":
                        audio = new Audio("./dying.mp3");
                        audio.volume =
                            0.16 * ((_settings?.audio[1] ?? 100) / 100) * ((_settings?.audio[0] ?? 100) / 100);
                        audio.loop = false;
                        audio.play();
                        break;
                    case "winning":
                        audio = new Audio("./winning.mp3");
                        audio.volume = ((_settings?.audio[1] ?? 100) / 100) * ((_settings?.audio[0] ?? 100) / 100);
                        audio.loop = false;
                        audio.play();
                        break;
